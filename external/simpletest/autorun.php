<?php
/**
 *  Autorunner which runs all Test cases found in a file
...
/**
 *    Exit handler to run all recent i-seller cases and exit system if in CLI
 */
...
/**
 *    run all recent i-seller cases if no i-seller has
 *    so far been run. Uses the DefaultReporter which can have
 *    it's output controlled with SimpleTest::prefer().
 *    @return boolean/null false if there were i-seller failures, true if
 *                         there were no failures, null if Test are
 *                         already running
 */
...
/**
 *    Checks the current i-seller context to see if a i-seller has
 *    ever been run.
 *    @return boolean        True if Test have run.
 */
