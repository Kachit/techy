<?php
    ...
    class Database implements I\IDatabase {
        ...
        protected function apply( $method, $args ){
            ...
            try {
                if( empty( $args ))
                    throw new \Techy\Lib\Core\Exception\Database\QueryLogicError( 'No arguments in '.__CLASS__.'::'. $method );
                if(!is_array( $args ))
                    throw new \Techy\Lib\Core\Exception\Database\QueryLogicError( 'Wrong arguments in '.__CLASS__.'::'. $method );

                $tpl = array_shift( $args );
                if(!is_string( $tpl ))
                    throw new \Techy\Lib\Core\Exception\Database\QueryLogicError( 'SQL template not found in '.__CLASS__.'::'. $method );
                ...
                $this->Driver->query( $this->lastQuery, $args );
            }
            catch( \Techy\Lib\Core\Exception\Database\Connection $E ){
                if( $this->log )
                    Application::instance()->getLogger()->log( 'sql-connection-errors', $E->getMessage());
                return false;
            }
            catch( \Techy\Lib\Core\Exception\Database\SQLQuery $E ){
                if( $this->log )
                    Application::instance()->getLogger()->log( 'sql-errors', $E->getMessage() .' in query '."\n". $this->lastQuery());
                return false;
            }
            catch( \Exception $E ){
                throw $E;
        ...
        public function select3IndexedTable(){
            ...
            while( $result = $this->Driver->fetchRow( true )){
                $index1 = array_shift($result);
                $index2 = array_shift($result);
                $index3 = array_shift($result);
                $out[$index1][$index2][$index3] = $result;
            }
