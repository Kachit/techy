<?php
    ...
    class Blitz extends \Blitz {
         ...
         * @param $spotId
         */
        public function __construct( \Techy\Lib\Core\Database\I\IConfig $Config, $spotId = null ){
            $this->Config = $Config;
            $this->spotId = $spotId;
            parent::__construct();
        }
        ...
        public function t( $table, $spotId = null ){
            if( preg_match( '#\W#', $table ))
                throw new \UnexpectedValueException( '`'. $table .'` is not valid identifier for table' );
            if(!$spotId )
                $spotId = $this->spotId;

            return $table . ( $spotId ? '_'. $spotId : '' );
        }
