<?php
    ...
    class Authorizer extends MemcachedBaseService implements \Techy\Lib\Core\Service\I\IAuthorizer {
        ...
        public function set( $key, array $data ){
            try {
                $this->client()->set( $this->name . $key, serialize( $data ), $this->expiration );
                return true;
            }
            catch( \Exception $E ){
        ...
        public function remove( $key ){
            try {
                $this->client()->delete( $this->name . $key );
                return true;
            }
            catch( \Exception $E ){
                Application::instance()->getLogger()->logException( $E );
